...
# All_Fighter compiles all Allied truck haul ticket data for import into Access database.
class All_Merger(object):
    ...
    def all_zipper(self):
        ...
        master_columns = ["Type", "Source", "TransNum", "DateIn", "TimeIn", "TruckNum", "DumpTixNum", "Material", "ItemNum", \
...
# RAC_Fighter compiles all Reliable truck haul ticket data for import into Access database.
class RAC_Merger(object):

    def __init__(self, rac_in, rac_out):
    ...
    def rac_zipper(self):
        ...
        # To date and export Merge.
        datestr = datetime.strftime(datetime.now(), "%m%d%Y")
        outpath = self.rac_out + "Merge_" + datestr + ".xlsx"
        writer = pd.ExcelWriter(outpath)
        df_og.to_excel(writer, sheet_name = "Sheet1")
        writer.save()
        print("A pan de quince días, hambre de tres semanas. \n")

    # rac_summary summarizes duh
    def rac_summary(self):
        
        datestr = datetime.strftime(datetime.now(), '%m%d%Y')
        ta_merge = 'N:\\Program Controls\\CTR Program Controls\\Truck Haul Program\\ReliableAsphalt-84271' \
                   + '\\Merge\\Merge_' + datestr + '.xlsx'
        df_merge = pd.read_excel(ta_merge)
        df_merge['UnitPrice'] = np.nan
        
        ta_up = 'N:\\Program Controls\\CTR Program Controls\\Truck Haul Program\\ReliableAsphalt-84271' \
                + '\\Merge\\PO84271_ItemUnitPrice_11252019.xlsx'
        ...
        outpath = "N:\\Program Controls\\CTR Program Controls\\Truck Haul Program\\ReliableAsphalt-84271\\Merge\\SummaryCheck_" \
                      + datestr + ".xlsx"
        writer = pd.ExcelWriter(outpath)
...
class Uti_Merger(object):
    ...
    def uti_zipper(self):
        ...
        # Link to where Merge is going to be saved.
        outpath = self.uti_out + "Merge_" + date_str + ".xlsx"
        writer = pd.ExcelWriter(outpath)
